...
#define PRINT_TIME 				1
#define SM_ARR_LEN			NUM_THREADS_PER_BLOCK_DIMENSION
#define TOL						1e-1
#define OMEGA 1.8
...
int idy = blockDim.y*blockIdx.y + threadIdx.y;
  if ((idx<arrLen)&&(idy<arrLen)){       // check array bounds
    int index = idx*arrLen+idy;          // convert 2D ID to 1D ID
...
//printf ("GPU: %f  CPU: %f Original: %f Err: %f\n",h_arrayGPU[i],h_arrayCPU[i], h_arrayOriginal[i],abs((h_arrayCPU[i] - h_arrayGPU[i])/h_arrayGPU[i]));
