...
__global__ void kernel_MMM (int arrLen, float* A, float* B, float* C) {
  for (int idx = blockDim.x*blockIdx.x + threadIdx.x; idx < arrLen; idx+=gridDim.x*blockDim.x){
    for (int idy = blockDim.y*blockIdx.y + threadIdx.y; idy < arrLen; idy+=gridDim.y*blockDim.y){
